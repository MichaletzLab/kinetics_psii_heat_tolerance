...
# Prepared by Nicole Bison (nicole.bison@ubc.ca), August 2025. Revised August 2026.
###...................................###
...
# Source data: one CSV per figure panel / table in source_data/, saved from the
# per-figure NEE source-data workbooks. NEE requires
# each figure to ship its own spreadsheet, so several figures draw on the same
# underlying data from different files; this is noted inline where it happens.
sd_dir <- "source_data"        # relative to this script's folder (data_code/)

# read one source-data CSV: column names run through make.names(), and literal
# "NA"/"" are read as missing
read_source <- function(file) {
  x <- read.csv(file.path(sd_dir, file), check.names = FALSE, stringsAsFactors = FALSE,
                fileEncoding = "UTF-8", na.strings = c("NA", "NaN", ""))
  names(x) <- make.names(names(x))
  x
}

# Load psii heat tolerance data                          # Fig 2a,b,c,d
leaf_data_all <- read_source("Bison_Source_Data_Fig2_a_b_c.csv")
#Get species,family counts
length(unique(leaf_data_all$species))
length(unique(leaf_data_all$family))

# Reference Tiso (deg C): the dashed line in Fig 2d and Fig 2f, and the threshold for the
# proportions quoted in the text (bottom of this script). 50.5 is the PSII (50.47) and the
# general-protein (50.54) bootstrap-mean Tiso from ED Table 2, both rounded to 0.1 deg C.
Tiso_ref <- 50.5

#### Fig 2a  ####
# species/genus in the source sheet are "Genus species"; V.PhyloMaker and the
# resulting tip labels use "Genus_species", so match that here.
sp_list <- leaf_data_all[,c(4,3,2)]
sp_list[[1]] <- gsub(" ", "_", sp_list[[1]])
sp_list[[2]] <- gsub(" ", "_", sp_list[[2]])
tree <- phylo.maker(sp_list, tree = GBOTB.extended, output.tree = T, r = 1)
tree3 <- tree$scenario.3

leaf_tips <- gsub(" ", "_", leaf_data_all$species)

ferns <- subset(leaf_tips, leaf_data_all$clade == "pteridophyte")
node_ferns <- findMRCA(tree3, tips = ferns)
node_ferns

gymnosperms <- subset(leaf_tips, leaf_data_all$clade == "gymnosperm")
node_gymnosperms <- findMRCA(tree3, tips = gymnosperms)
node_gymnosperms

angiosperms <- subset(leaf_tips, leaf_data_all$clade == "angiosperm")
node_angiosperms <- findMRCA(tree3, tips = angiosperms)
node_angiosperms
...
fig2a <- ggtree(tree3, layout = "circular") +
  ...
  # Foreground highlights for the three clades. The node ids are the MRCA of
  # each clade computed from the tree above (they were previously hard-coded as
  # 326 / 340 / 180, which silently pointed at the wrong nodes once the tree
  # changed - it is now 327 / 341 / 180 with the cleaned data).
  geom_hilight(node = node_gymnosperms, fill = '#003f5c', type = "rect", alpha = 1) +
  geom_hilight(node = node_ferns,       fill = '#bc5090', type = "rect", alpha = 1) +
  geom_hilight(node = node_angiosperms, fill = '#ffa600', type = "rect", alpha = 1) +
...
fig2b <- ggplot(leaf_data_all, aes(x = clade, y = ea_kJ_mol, color = clade)) +
...
#### Fig 2c ####                                        # He & Bischof (2003) proteins
protein_only <- read_source("Bison_Source_Data_Fig2_c.csv")
protein_only <- protein_only[,c(2,3,6)]

colnames(protein_only) <- c("species","ea_kJ_mol","a_s-1")
protein_only$clade = "protein"


leaf_data_proteins <- leaf_data_all[,c(1,4:6)]
leaf_data_proteins$"`a_s-1`" <- exp(leaf_data_proteins$ln_a)
leaf_data_proteins <- leaf_data_proteins[,c(1,2,3,5)]
colnames(leaf_data_proteins)
colnames(protein_only)
colnames(leaf_data_proteins)[colnames(leaf_data_proteins) == "`a_s-1`"] <- "a_s-1"
leaf_data_proteins <- rbind(leaf_data_proteins,protein_only)
leaf_data_proteins$lnA <- log(leaf_data_proteins$`a_s-1`)
...
fig2c<- ggplot(leaf_data_proteins, aes(x = ea_kJ_mol, y = lnA, color = clade)) +
  ...
  ylab(expression(ln(italic(A))~"(unitless)"))+
...
# r-squared of ln(A) against Ea (quoted in the Fig 2c caption and the Results):
# PSII, n = 177, r2 = 0.9994; proteins, n = 23 (the 8 proteins without an A are dropped), r2 = 0.9538
r2_fig2c <- leaf_data_proteins %>%
  filter(is.finite(lnA)) %>%
  mutate(group = ifelse(clade == "protein", "protein", "PSII")) %>%
  group_by(group) %>%
  summarise(n = n(), r2 = cor(ea_kJ_mol, lnA)^2)
print(as.data.frame(r2_fig2c), digits = 4)


### Fig 2d ####
#create a data frame of temperatures (40°C to 70°C, in Kelvin)
arrhenius_plot <- leaf_data_all[,c(1,4:6)]
arrhenius_plot$ln_a_pred <- 0.3716727*arrhenius_plot$ea_kJ_mol + 1
...
fig2d <- ggplot() +
  ...
  scale_x_continuous(
    name = expression("\u{2212}1 /"~italic(RT)~"(mol"~kJ^{-1}*")"),
  ...
  geom_vline(xintercept = -1 / (R * (Tiso_ref + 273.15)), col = "black", linetype = "dashed", size = 1) +
...
##### Fig 2e ####
# Literature compilation. Same data as ED Fig 4 (reloaded there from its own
# file, Bison_Source_Data_ED_Fig4_histogram_data.csv).
lit_review <- read_source("Bison_Source_Data_Fig2_e_f.csv")
...
fig2f<-ggplot(lit_review, aes(x=heat_tolerance)) +
  ...
  geom_vline(xintercept = Tiso_ref, col="black", linetype="dashed", linewidth=1.2)+
...
#### below not used in figure, but shows corresponding plots in Fig2f at the spcies level
# Fig 2f at the species level: one mean Tcrit per species
lit_review_species <- lit_review %>%
  group_by(species) %>%
  summarise(heat_tolerance = mean(heat_tolerance, na.rm = TRUE), .groups = "drop")

fig2f_species <- ggplot(lit_review_species, aes(x = heat_tolerance)) +
  geom_histogram(binwidth = 2, fill = "#539987", color = "black", alpha = 0.9) +
  geom_vline(xintercept = Tiso_ref, col = "black", linetype = "dashed", linewidth = 1.2) +
...
ggsave("./figures/fig2.pdf", last_plot(), height = 25, width = 25, units = "cm")


####### Figure 3 #######
#Load kinetics, heat tolerance data from seasonal dataset
# (identical to the ED Fig 3 trait data, reloaded there as `d`)
df_seasons <- read_source("Bison_Source_Data_Fig3_a_c_e_g.csv")
df_seasons$season <- factor(df_seasons$season, levels = c("spring","summer","fall"))
# leaves per season (n = 21 / 30 / 27; 78 in total)
table(df_seasons$season)

#Load climate data (trait rows joined to cumulative heat-stress days)
df_seasons_climate <- read_source("Bison_Source_Data_Fig3_b_d_f.csv")

#### Fig 3a ####
fig3a<- ggplot(df_seasons, aes(x= season, y = tcrit_C))+
...
#### Fig 3b ####
fig3b <- ggplot(df_seasons_climate, aes(x= Heat_Stress_Days_Cumulative, y = tcrit_C))+
  scale_color_manual(values = c("#A8E6CF","#FFD54F","#FF8C42"))+
...
#### Fig 3c ####
fig3c <- ggplot(df_seasons, aes(x= season, y = ea_kJ_mol))+
  scale_color_manual(values = c("#A8E6CF","#FFD54F","#FF8C42"))+
  geom_jitter(aes(color = season, shape = species),size =2,alpha =0.5,width =0.2)+ 
  geom_boxplot(aes(group = season),outlier.shape = NA, width =0.5,alpha=0.5,size=1, fill = NA)+
  scale_fill_manual(values = c("#A8E6CF","#FFD54F","#FF8C42"))+
...
#### Fig 3d ####
fig3d <-ggplot(df_seasons_climate, aes(x= Heat_Stress_Days_Cumulative, y = ea_kJ_mol))+
  scale_color_manual(values = c("#A8E6CF","#FFD54F","#FF8C42"))+
  geom_jitter(aes(color = season),size =2,alpha =0.5,width =0.2)+ 
  geom_smooth(aes(fill= season), method = "lm", color = "black", fill = "black")+
  scale_fill_manual(values = c("#A8E6CF","#FFD54F","#FF8C42"))+
...
#### Fig 3e ####
fig3e <- ggplot(df_seasons, aes(x= season, y = ln_a_unitless))+
  scale_color_manual(values = c("#A8E6CF","#FFD54F","#FF8C42"))+
  geom_jitter(aes(color = season, shape = species),size =2,alpha =0.5,width =0.2)+ 
  geom_boxplot(aes(group = season),outlier.shape = NA, width =0.5,alpha=0.5,size=1, fill = NA)+
  scale_fill_manual(values = c("#A8E6CF","#FFD54F","#FF8C42"))+
  xlab("Season")+
  ylab(expression(ln(italic(A))))+
  theme(panel.background = element_blank(),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.line = element_line(color = "black"),
...
#### Fig 3f ####
fig3f <-ggplot(df_seasons_climate, aes(x= Heat_Stress_Days_Cumulative, y = ln_a_unitless))+
  scale_color_manual(values = c("#A8E6CF","#FFD54F","#FF8C42"))+
  geom_jitter(aes(color = season),size =2,alpha =0.5,width =0.2)+ 
  scale_fill_manual(values = c("#A8E6CF","#FFD54F","#FF8C42"))+
  xlab("Cumulative days > 21°C")+
...
fig3g <- ggplot(df_seasons, aes(x = ea_kJ_mol, y =ln_a_unitless, color = season)) +
  scale_color_manual(values = c("#A8E6CF","#FFD54F","#FF8C42")) +
...
# r-squared of ln(A) against Ea for the two fitted groups in Fig 3g
# (spring: n = 21, r2 = 0.9997; summer + autumn: n = 57, r2 = 0.9991)
r2_fig3g <- df_seasons %>%
  group_by(season_lumped) %>%
  summarise(n = n(), r2 = cor(ea_kJ_mol, ln_a_unitless)^2)
print(as.data.frame(r2_fig3g), digits = 4)
...
bootstrap_sma <- function(data, n_boot = 1000) {
  replicate(n_boot, {
    ...
    model <- try(sma(ln_a_unitless ~ ea_kJ_mol, data = sample_data), silent = TRUE)
...
##### Fig 3h ####
# HSP melting temperatures (ProThermDB, A. thaliana) drawn as dashed reference
# lines. Same 12-row table as ED Fig 4 and ED Table 5 (here from its own file).
hsp_fig3 <- read_source("Bison_Source_Data_Fig3_h.csv")[, c(2, 29)]
colnames(hsp_fig3) <- c("PROTEIN", "Tm_C")
hsp_fig3 <- hsp_fig3 %>%
  mutate(HSP_group = case_when(grepl("20", PROTEIN) ~ "HSP20",
                               grepl("60", PROTEIN) ~ "HSP60",
                               grepl("70", PROTEIN) ~ "HSP70",
                               grepl("90|91", PROTEIN) ~ "HSP90",
                               TRUE ~ NA_character_),
         Tm_C = as.numeric(sub(" .*", "", Tm_C))) %>%
  filter(!is.na(HSP_group)) %>%
  group_by(HSP_group) %>% summarise(Tm = mean(Tm_C), .groups = "drop")
Tm_hsp <- setNames(hsp_fig3$Tm, hsp_fig3$HSP_group)   # HSP20 55.1, HSP60 58.7, HSP70 43.7, HSP90 47.4

fig3h <-ggplot(seasons_Tiso, aes(x= season, y = Tiso))+
  geom_jitter(aes(color = season),size =2,alpha =0.1,width =0.2)+
  geom_boxplot(aes(group = season),outlier.shape = NA, width =0.5,alpha=0.5,size=1, fill = NA)+
  scale_color_manual(values = c("#A8E6CF","#FFD54F","#FF8C42"))+
  geom_hline(yintercept = Tm_hsp["HSP20"], col="black", linetype="dashed", size=1)+
  geom_hline(yintercept = Tm_hsp["HSP60"], col="black", linetype="dashed", size=1)+
  geom_hline(yintercept = Tm_hsp["HSP70"], col="black", linetype="dashed", size=1)+
  geom_hline(yintercept = Tm_hsp["HSP90"], col="black", linetype="dashed", size=1)+
...
fig3<-ggarrange(fig3a,fig3b, fig3c,fig3d, fig3e, fig3f,fig3g,fig3h,
                ...
                ncol = 2, nrow = 4)
fig3
#ggsave("./figures/fig3.pdf", last_plot(), height = 26, width = 15, units = "cm")
...
#### Table 1####                             # PSII + protein + lipid-bilayer Ea
leaf_proteins_lipids <- read_source("Bison_Source_Data_ED_Table1.csv")

#summarize across clades and type
df_summary_clade <- leaf_data_all %>%
  group_by(clade) %>%
  summarise(
    n = n(),
    min = min(ea_kJ_mol),
    max = max(ea_kJ_mol),
    median = median(ea_kJ_mol),
    mean = mean(ea_kJ_mol),
    sd = sd(ea_kJ_mol),
    se = sd / sqrt(n),
...
# ED Table 1, PSII rows by clade (n = 148 / 15 / 14)
print(as.data.frame(df_summary_clade), digits = 5)
...
# ED Table 1, lipid / protein / all-PSII rows
print(as.data.frame(df_summary), digits = 5)

#### Table 2 ####
#Get general data and protein data
leaf_data_proteins <- subset(leaf_proteins_lipids,leaf_proteins_lipids$type != "Lipid bilayer")
leaf_data_proteins <- leaf_data_proteins[,c(4,2,3)]

#Get seasonal data by season
spring <- df_seasons %>%
  filter(season == "spring") %>%
  dplyr::select(ea_kJ_mol, ln_a_unitless)
colnames(spring) <- c("ea_kJ_mol","ln_a_unitless")
spring$type <- "spring"

summer <- df_seasons %>%
  filter(season == "summer") %>%
  dplyr::select(ea_kJ_mol, ln_a_unitless)
colnames(summer) <- c("ea_kJ_mol","ln_a_unitless")
summer$type <- "summer"

fall <- df_seasons %>%
  filter(season == "fall") %>%
  dplyr::select(ea_kJ_mol, ln_a_unitless)
colnames(fall) <- c("ea_kJ_mol","ln_a_unitless")
fall$type <- "fall"
...
# `leaf_proteins_seasons` assembled above == Bison_Source_Data_ED_Table3.csv
# (type, ea_kJ_mol, ln_a_unitless); read the shipped file directly.
leaf_proteins_seasons <- read_source("Bison_Source_Data_ED_Table3.csv")
sma_model <- sma(ln_a_unitless ~ ea_kJ_mol * type, data = leaf_proteins_seasons, multcomp = TRUE)
sma_model


##### Table 4 #######                        # Point Atkinson daily climate, 2023
climate_raw <- read_source("Bison_Source_Data_ED_Table4.csv")
# Date/Time is ISO 8601 text (YYYY-MM-DD); "m/d/YYYY" is also accepted
climate_raw$date <- format(
  as.Date(climate_raw$Date.Time, tryFormats = c("%Y-%m-%d", "%m/%d/%Y")),
  "%Y%m%d")
...
for (thresh in thresholds) {
  ...
  model_data <- df_seasons_climate %>%
    dplyr::select(date, ea_kJ_mol, species) %>%
    left_join(climate_thresh, by = "date") %>%
    filter(!is.na(ea_kJ_mol), !is.na(Heat_Stress_Days_Cumulative), !is.na(species))
  
  # Fit linear model with interaction
  if (nrow(model_data) >= 5) {
    lm_model <- lm(ea_kJ_mol ~ Heat_Stress_Days_Cumulative, data = model_data)
...
##### Table 5 #######                        # ProThermDB A. thaliana HSP melting temps
prothermdb_a_thaliana_hsp <- read_source("Bison_Source_Data_ED_Table5.csv")
...
# ED Table 5 / Fig 3h / ED Fig 4 reference lines: HSP20 55.1, HSP60 58.7, HSP70 43.7, HSP90 47.4
print(as.data.frame(summary_hsp), digits = 4)

#### Figure 1 ####                           # protein vs lipid-bilayer Ea
# Same protein + lipid rows as ED Table 1 (type != "psii"), shipped as their
# own file for this figure.
protein_lipids <- read_source("Bison_Source_Data_ED_Fig1.csv")
...
ggplot(protein_lipids_summary, aes(x = type, y = mean,fill = type,color = type))+
  ...
  theme(panel.background = element_blank(),  
        ...
        legend.title = element_blank(),
        legend.text = element_text(size = 12))
#ggsave("./figures/ED_Fig1.pdf",last_plot(), height = 12, width =12, units = "cm")


#### Figure  3####
# Seasonal trait data (same 78 rows as main Fig 3a,c,e,g) crossed against the
# Point Atkinson climate over 1-120 day averaging windows.
d <- read_source("Bison_Source_Data_ED_Fig3_trait_data.csv")
...
fit_temp_models <- function(data, temp_cols = as.character(1:120)) {
  responses <- c("tcrit_C", "ea_kJ_mol", "ln_a_unitless")
...
#ggsave("./figures/ED_Fig3.pdf",last_plot(), height = 12, width =15, units = "cm")

#### Figure 4 ####
# Literature-compilation Tcrit histogram - same data as Fig 2e,f, from this
# figure's own file; HSP melting-temp reference lines from ED Table 5.
lit_review_ed4 <- read_source("Bison_Source_Data_ED_Fig4_histogram_data.csv")
Tm <- setNames(summary_hsp$mean_Tm, summary_hsp$HSP_group)

ggplot(lit_review_ed4, aes(x=heat_tolerance)) +
  #geom_histogram( binwidth=2, fill="#93B7BE", color="black", alpha=0.9) +
  geom_histogram( binwidth=2, fill="#539987", color="black", alpha=0.9) +
  geom_vline(xintercept = Tm["HSP90"], col="black", linetype="dashed", linewidth=1.2)+ #HSP90 ~47.4
  geom_vline(xintercept = Tm["HSP70"], col="black", linetype="dashed", linewidth=1.2)+ #HSP70 ~43.7
  geom_vline(xintercept = Tm["HSP60"], col="black", linetype="dashed", linewidth=1.2)+ #HSP60 ~58.7
  geom_vline(xintercept = Tm["HSP20"], col="black", linetype="dashed", linewidth=1.2)+ #HSP20 ~55.1
  ...
  theme(panel.background = element_blank(),  # Removes the background color
        ...
        #axis.text.x = element_blank(),
        legend.position = "none")
#ggsave("./figures/ED_Fig4.pdf",last_plot(), height = 12, width =15, units = "cm")


##### Statistical tests ####
##### Figure 2 a
#Is Ea phylogenetically structured?
trait <- setNames(leaf_data_all$ea_kJ_mol, gsub(" ", "_", leaf_data_all$species))
phylosig(tree3, trait, method="lambda", test=TRUE, nsim=999)

##### Figure 2 b

#Variation in Ea across clades
anova_model <- aov(ea_kJ_mol ~ clade, data = leaf_data_all)
summary(anova_model)
...
leaf_protein <- subset(leaf_proteins_lipids,
                       type != "Lipid bilayer")

t.test(ea_kJ_mol ~ type, data = leaf_protein)
...
#fit ANOVA
anova_model <- aov(ea_kJ_mol ~ clade, data = leaf_protein)
...
anova_model <- aov(ea_kJ_mol ~ type, data = leaf_lipid)
summary(anova_model)
...
#fit ANOVA
anova_model <- aov(ea_kJ_mol ~ clade, data = leaf_lipid)
...
##### Figure 2c
#Is there a diff in slopes between proteins and psii?
protein_leaf <- subset(leaf_proteins_lipids, leaf_proteins_lipids$type != "Lipid bilayer")
slope_test <- slope.com(
  y = protein_leaf$ln_a,
  x = protein_leaf$ea,
  groups = protein_leaf$type
)
...
protein_leaf <- subset(leaf_proteins_lipids, leaf_proteins_lipids$type != "Lipid bilayer")
sma_model <- elev.com(y=protein_leaf$ln_a, x=protein_leaf$ea_kJ_mol, groups=protein_leaf$type)
sma_model


##### Figure 2f
## Proportion of Tcrit values at or above Tiso_ref (50.5 C); below = 1 - this.
## (One species mean and two observations equal 50.5 exactly, so ">=" and ">" differ slightly.)
# Total observations
length(subset(lit_review$species, lit_review$heat_tolerance >= Tiso_ref))/length(lit_review$species)

#Grouped by species
species <- lit_review %>%
  group_by(species) %>%
  summarise(heat_tolerance = mean(heat_tolerance, na.rm = T))
length(subset(species$species, species$heat_tolerance >= Tiso_ref))/length(species$species)
...
#####Figure 3
# column names in the source files are tcrit_C / ea_kJ_mol / ln_a_unitless
#What is average Tcrit by season?
df_seasons %>%
  group_by(season) %>%
  summarise(mean_Tcrit = mean(tcrit_C, na.rm = TRUE))

#Do parameters vary seasonally?
#Tcrit
anova_model <- aov(tcrit_C ~ season, data = df_seasons)
summary(anova_model)
...
#Ea
res_aov <- aov(ea_kJ_mol ~ season,
               data = df_seasons
)
summary(res_aov)

#ln(A)
res_aov <- aov(ln_a_unitless ~ season,
               data = df_seasons
)
summary(res_aov)

#Do parameters correlate with cumulative heat exposure?
summary(lm(tcrit_C~Heat_Stress_Days_Cumulative, data=df_seasons_climate))
summary(lm(ea_kJ_mol~Heat_Stress_Days_Cumulative, data=df_seasons_climate))
summary(lm(ln_a_unitless~Heat_Stress_Days_Cumulative, data=df_seasons_climate))
